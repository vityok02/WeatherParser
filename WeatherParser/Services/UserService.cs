...
public class UserService : IUserService
{
    ...
    public async Task<User> GetUserWithLocationsAsync(long userId, CancellationToken cancellationToken)
    {
        _memoryCache.TryGetValue(userId, out User? user);
        if (user is not null)
        {
            return user;
        }
    ...
    public async Task AddUserToDbAsync(User user, CancellationToken cancellationToken)
    {
